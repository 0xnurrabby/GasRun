...
// KV keys
const KV_PREFIX = "lanerunner:lb:v5";
...
async function backfillTwoWeeks(now, deadlineMs) {
  ...
  // Safety overlap (do NOT shift the week boundary earlier; we only widen ranges).
  const BOUNDARY = curFrom;
  const BUFFER = 2000n;

  // Start a little earlier for safety.
  const curFromBuffered = (BOUNDARY > BUFFER) ? (BOUNDARY - BUFFER) : 0n;
  const prevFromBuffered = (prevFrom > BUFFER) ? (prevFrom - BUFFER) : 0n;

  // End slightly after boundary so we never miss boundary logs (week filtering keeps correctness).
  let prevTo = 0n;
  if (BOUNDARY > 0n) {
    prevTo = (BOUNDARY - 1n) + BUFFER;
    if (prevTo > latest) prevTo = latest;
  }

  curFrom = curFromBuffered;
  prevFrom = prevFromBuffered;
...
async function applyLogsToWeekMap(state, weekMs, logs) {
  ...
  // Dedupe logs within a single scan batch. Some providers (and explorer APIs) can return duplicates,
  // which would otherwise inflate points.
  const seen = new Set();
  ...
  for (const l of logs) {
    const tx = (l.transactionHash || l.txHash || "").toLowerCase();
    const li = normIndex(l.logIndex);
    const bn = normIndex(l.blockNumber);
    const key = tx && li !== null ? `${tx}:${li}` : `${bn || ""}:${tx || ""}:${li || ""}`;
