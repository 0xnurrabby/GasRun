...
function updateShareUI() {
  ...
  const btn = $("#shareStat");
  if (btn) {
    btn.disabled = used >= SHARE_MAX;
    btn.classList.toggle("disabled", used >= SHARE_MAX);
  }
}
...
async function shareCurrentWeeklyStat() {
  ...
  try {
    ...
    if (s && s.actions && typeof s.actions.composeCast === "function") {
      ...
      // Fallback reward timer (Base): grant reward 5s after click.
      // We will cancel this timer if we can reliably detect a cancel,
      // or if we detect a real cast id/hash (Farcaster).
      ...
      // Otherwise: keep the easy Base rule (reward after 5s).
...
async function openLeaderboardsView(forceRefresh = false) {
  ...
  openSheet(
    "Leaderboards",
    `
    <div class="copy boardsStatus" id="boardsStatus" style="opacity:.9">Loading on-chain leaderboard…</div>
