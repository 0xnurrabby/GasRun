...
function awardShareBonusIfNewCast(cast) {
  ...
  toast(`+${fmtPts(SHARE_REWARD_POINTS)} Saved points! Deposit to leaderboard 💚`, 2600);
  return true;
}
...
async function shareCurrentWeeklyStat() {
  ...
  const used = getShareCount();
  if (used >= SHARE_MAX) {
    toast("Share limit reached (10/10).");
    updateShareUI();
    return;
  }
  ...
  try {
    ...
    if (s && s.actions && typeof s.actions.composeCast === "function") {
      ...
      // If user cancels, cast can be null.
  ...
  toast("Tip: Share from Base App/Farcaster to earn +10k bonus.");
  const intent =
