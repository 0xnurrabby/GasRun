...
function render() {
  ...
  for (const c of game.coins) {
    ...
    if (c.kind !== "coin") {
      ctx.fillStyle = "rgba(0,0,0,0.65)";
      ctx.font = `bold ${Math.max(10, c.r + 2)}px "AmericanCaptainPatrius", ui-sans-serif, system-ui`;
...
if (game.over) {
  ...
  ctx.font = "800 20px \"AmericanCaptainPatrius\", system-ui, -apple-system, Segoe UI, Roboto";
  ctx.fillText("Crash!", boxX + boxW / 2, boxY + yOff + 40);

  ctx.globalAlpha = 0.85 * ease;
  ctx.fillStyle = "rgba(255,255,255,0.78)";
  ctx.font = "600 13px \"AmericanCaptainPatrius\", system-ui, -apple-system, Segoe UI, Roboto";
