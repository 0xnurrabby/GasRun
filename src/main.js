...
async function commitWeeklyOnchain() {
  ...
  try {
    ...
    // 2) If paymaster-related failure → retry wallet_sendCalls without paymaster
    // 3) If wallet_sendCalls unsupported → fallback to eth_sendTransaction
    ...
    const fallbackEthSendTransaction = async () => {
      const txParams = { from: account, to: CONTRACT, data, value: "0x0" };
    ...
    if ((isLikelyPaymasterFailure || proxyLooksBad) && params?.capabilities?.paymasterService) {
      try {
        const paramsNoPm = {
          ...params,
          capabilities: { ...(params.capabilities || {}) },
        };
        delete paramsNoPm.capabilities.paymasterService;
