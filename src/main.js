...
function bindSwipeControls(targetEl) {
  ...
  function onMove(ev) {
    if (!tracking || fired || pointerId !== ev.pointerId) return;
    if (game.over) return;
  ...
  function onUp(ev) {
    ...
    // Optional: quick tap on left/right half as a fallback (feels great on mobile)
    if (!game.over && !fired && dt < 220 && ax < 10 && ay < 10) {
      const r = targetEl.getBoundingClientRect();
