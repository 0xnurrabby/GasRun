...
// Optional: ERC-7677 Paymaster service URL (CDP Paymaster endpoint).
// We read it from /api/config (serverless) so you can configure it via Vercel env vars.
// Fallback: you can also set window.__PAYMASTER_URL__ in index.html.
let _paymasterUrlCache = undefined;

async function getPaymasterUrl() {
  if (_paymasterUrlCache !== undefined) return _paymasterUrlCache;

  // 1) window override (for fully static deployments)
  try {
    const w = window;
    const v = w && typeof w.__PAYMASTER_URL__ === "string" ? w.__PAYMASTER_URL__.trim() : "";
    if (v) {
      _paymasterUrlCache = v;
      return _paymasterUrlCache;
    }
  } catch {}

  // 2) serverless config (recommended)
  try {
    const res = await fetch("/api/config", { cache: "no-store" });
    const j = await res.json().catch(() => ({}));
    const v = typeof j?.paymasterUrl === "string" ? j.paymasterUrl.trim() : "";
    _paymasterUrlCache = v || null;
    return _paymasterUrlCache;
  } catch {
    _paymasterUrlCache = null;
    return _paymasterUrlCache;
  }
}

async function maybeAttachPaymasterCapability(provider, chainIdHex, paramsObj) {
  const url = await getPaymasterUrl();
  if (!url) return false;

  // Check if the connected wallet supports paymasterService on this chain.
  // Spec: EIP-5792 + ERC-7677.
  try {
    const caps = await provider.request({
      method: "wallet_getCapabilities",
      params: [account]
    });
    const supported = !!caps?.[chainIdHex]?.paymasterService?.supported;
    if (!supported) return false;

    paramsObj.capabilities = paramsObj.capabilities || {};
    paramsObj.capabilities.paymasterService = { url };
    return true;
  } catch {
    return false;
  }
...
async function commitWeeklyOnchain() {
  ...
  try {
    ...
    // If a Paymaster URL is configured and the wallet supports it, request sponsorship.
    // This keeps the game working even on wallets that don't support paymasterService.
    await maybeAttachPaymasterCapability(p, chainId, params);
