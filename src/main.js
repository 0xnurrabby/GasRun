...
// ---------- Robust async helpers (prevents stuck UI on slow/buggy clients) ----------
...
function resetWeb3WarmCache() {
  // NOTE: dynamic imports can't be truly cancelled; this just allows retries
  // instead of staying stuck on a never-resolving promise.
  web3WarmReady = false;
  web3WarmPromise = null;
  _viemPromise = null;
  _oxPromise = null;
  // Do not clear already-resolved functions if present.
  // If they exist, warmWeb3Deps will still mark ready quickly.
}

async function warmWeb3DepsOrThrow() {
  if (web3WarmReady) return;
  try {
    await withTimeout(warmWeb3Deps(), 9000, "Preparing") ;
    if (!web3WarmReady) throw new Error("Web3 deps not ready");
  } catch (e) {
    resetWeb3WarmCache();
    throw new Error(
      "Preparing took too long (slow network/device). Please try again."
    );
  }
}

// Abortable fetch helper for views like Leaderboards.
async function fetchJsonWithTimeout(url, { signal, timeoutMs = 12000, ...opts } = {}) {
...
// =====================================================
// Global in-flight guards + cancel helpers
// (prevents stuck buttons when users back out during "Preparing…")
// =====================================================
let boardsAbortCtrl = null;
let _commitAbortCtrl = null;
let _commitStage = "idle"; // idle | preflight | wallet

function cancelPendingUi() {
  // Leaderboards fetch
  try {
    if (boardsAbortCtrl) boardsAbortCtrl.abort();
  } catch {}
  boardsAbortCtrl = null;
  // Allow re-open even if an old fetch is still hanging.
  boardsInFlight = false;

  // Commit warm-up phase (safe to cancel only before wallet UI)
  try {
    if (_commitAbortCtrl && _commitStage === "preflight") _commitAbortCtrl.abort();
  } catch {}

  // Share compose (best-effort)
  _shareInFlight = false;
}

function assertNotAborted(signal) {
  if (signal && signal.aborted) throw new Error("__CANCEL__");
}
...
// LEADERBOARD API
// =====================================================
async function fetchLeaderboard({ refresh = false, names = false, signal } = {}) {
  const qs = new URLSearchParams();
  if (refresh) qs.set("refresh", "1");
  qs.set("names", names ? "1" : "0");

  const url = `/api/leaderboard?${qs.toString()}`;
  const { json: j } = await fetchJsonWithTimeout(url, {
    cache: "no-store",
    signal,
    timeoutMs: 14000
  });
...
async function commitWeeklyOnchain() {
  ...
  // Abort controller: lets us cancel the *preflight* if the user backs out.
  _commitAbortCtrl = new AbortController();
  _commitStage = "preflight";
  const commitSignal = _commitAbortCtrl.signal;
  ...
  assertNotAborted(commitSignal);

  try {
    await withTimeout(ensureBase(), 12000, "Switching chain");
    assertNotAborted(commitSignal);

    // Ensure web3 encoders + builder dataSuffix are available (prefetched in background when possible)
    await warmWeb3DepsOrThrow();
    assertNotAborted(commitSignal);

    const p = await withTimeout(getProvider(), 6000, "Wallet");
    if (!p) throw new Error("No provider");

    // From here on we expect wallet UI / chain calls.
    _commitStage = "wallet";

    const chainId = await withTimeout(
      p.request({ method: "eth_chainId", params: [] }),
      15000,
      "Wallet"
    );
    ...
    try {
      await withTimeout(p.request({ method: "wallet_sendCalls", params: [params] }), 70000, "Wallet");
    } catch (e) {
    ...
    if (String(e?.message || "") === "__CANCEL__") return;
    toast(e?.message ? String(e.message) : "Commit failed");
  } finally {
    commitInFlight = false;
    _commitStage = "idle";
    _commitAbortCtrl = null;
    if (commitBtn) {
...
async function openLeaderboardsView(forceRefresh = false) {
  ...
  // Cancel any previous boards load so the button never gets stuck.
  try {
    if (boardsAbortCtrl) boardsAbortCtrl.abort();
  } catch {}
  boardsAbortCtrl = new AbortController();
  boardsInFlight = true;
  ...
  try {
    const data = await fetchLeaderboard({ refresh: forceRefresh, names: false, signal: boardsAbortCtrl.signal });
    ...
    // If user backed out / closed sheet, silently stop.
    const msg = String(e?.message || "");
    if (boardsAbortCtrl?.signal?.aborted || msg === "__CANCEL__" || String(e?.name || "") === "AbortError") {
      return;
    }
