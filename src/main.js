...
// Share counts are stored per-week, per-user (FID when available).
function _shareCountKey(weekLabel) {
  const fid =
    (sdk && sdk.context && sdk.context.user && typeof sdk.context.user.fid === "number")
      ? sdk.context.user.fid
      : 0;
  return `w3r_share_${String(weekLabel || "week")}_fid${fid}`;
}

function getShareCount(weekLabel) {
  return Number(localStorage.getItem(_shareCountKey(weekLabel)) || "0");
}

function setShareCount(weekLabel, n) {
  localStorage.setItem(_shareCountKey(weekLabel), String(Math.max(0, Math.floor(n))));
}

function _claimedCastsKey() {
  return "w3r_share_claimed_casts";
}

function _getClaimedCasts() {
  try {
    return JSON.parse(localStorage.getItem(_claimedCastsKey()) || "{}") || {};
  } catch (_) {
    return {};
  }
}

function _markCastClaimed(hash) {
  if (!hash) return;
  const m = _getClaimedCasts();
  m[String(hash)] = 1;
  localStorage.setItem(_claimedCastsKey(), JSON.stringify(m));
}

function _isCastClaimed(hash) {
  if (!hash) return false;
  const m = _getClaimedCasts();
  return !!m[String(hash)];
}

function updateShareMetaUI() {
  const week = lastWeeklyShareCtx?.weekLabel || "week";
  const used = getShareCount(week);
  const usedEl = document.getElementById("shareUsed");
  const maxEl = document.getElementById("shareMax");
  if (usedEl) usedEl.textContent = String(Math.min(used, SHARE_MAX));
  if (maxEl) maxEl.textContent = String(SHARE_MAX);
  const btn = document.getElementById("shareStat");
  if (btn) btn.disabled = used >= SHARE_MAX;
}
...
function _buildShareUrl(ctx) {
  ...
  // Nonce used for post-claim (reward only after a real cast exists).
  const claim =
    (ctx && ctx.claim) ||
    `${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 10)}`;
  if (ctx) ctx.claim = claim;
  p.set("claim", claim);
  try {
    const fid =
      (sdk && sdk.context && sdk.context.user && typeof sdk.context.user.fid === "number")
        ? sdk.context.user.fid
        : 0;
    if (fid) p.set("fid", String(fid));
  } catch (_) {}

  if (ctx.weekLabel)if (ctx.weekLabel) p.set("week", ctx.weekLabel);
...
async function shareCurrentWeeklyStat() {
  if (!account) {
    toast("Connect wallet to share your stat.");
    return;
  }
  if (!lastWeeklyShareCtx || lastWeeklyShareCtx.rank == null) {
    toast("No weekly points yet—save points on-chain first!");
    return;
  }
  const week = lastWeeklyShareCtx.weekLabel || "week";
  const used = getShareCount(week);
  if (used >= SHARE_MAX) {
    toast(`Max shares reached (${SHARE_MAX}/${SHARE_MAX})`);
    return;
  }
  ...
  try {
    ...
    if (s && s.actions && typeof s.actions.composeCast === "function") {
      // Prefer the native action so it works across clients.
      await s.actions.composeCast({
        text,
        // Single embed to avoid duplicate cards.
        embeds: [shareUrl]
      });
      toast("Posted? Tap your GasRun card to return and claim +10,000 Saved points.", 3600);
      return;
    }
  } catch (_) {
  ...
  // Fallback: open Warpcast composer intent (works in normal browsers).
  const intent =
...
// Claim share reward only when the app is opened from a real cast that embeds our share URL.
// This avoids rewarding users who open the composer but never post.
async function maybeClaimShareReward() {
  try {
    const s = await ensureSdk().catch(() => null);
    const ctx =
      (s && s.context) ||
      (s && s.actions && (await s.actions.getContext?.())) ||
      null;
    const loc = ctx?.location || ctx?.context?.location || null;
    if (!loc || (loc.type !== "cast_embed" && loc.type !== "cast_share")) return;

    const cast = loc.cast;
    const castHash = cast?.hash;
    if (!castHash || _isCastClaimed(castHash)) return;

    // Only allow the cast author to claim.
    const viewerFid = ctx?.user?.fid || ctx?.context?.user?.fid || null;
    const authorFid = cast?.author?.fid || null;
    if (viewerFid && authorFid && Number(viewerFid) !== Number(authorFid)) return;

    const embeds = Array.isArray(cast?.embeds) ? cast.embeds : [];
    const match = embeds.find((u) => typeof u === "string" && u.includes("/api/share?"));
    if (!match) return;

    let url;
    try { url = new URL(match); } catch (_) { return; }
    const weekLabel = url.searchParams.get("week") || "week";

    // Enforce max shares.
    const used = getShareCount(weekLabel);
    if (used >= SHARE_MAX) {
      _markCastClaimed(castHash);
      return;
    }
    ...
    setShareCount(weekLabel, used + 1);
    updateShareMetaUI();

    _markCastClaimed(castHash);

    toast("✅ Shared! +10,000 Saved points added. Now deposit to the leaderboard to show your rank!", 4200);
  } catch (_) {
    // no-op
  }
}
...
async function openLeaderboardsView(forceRefresh = false) {
  ...
  openSheet(
    ...
    <div class="btnRow" style="margin-top:12px">
      <button class="pill" id="backMenu">Back</button>
      <div class="shareWrap">
        <button class="pill share" id="shareStat">Share<br/>your stat</button>
        <div class="shareMeta" id="shareMeta">Share <span class="used" id="shareUsed">0</span>/<span class="max" id="shareMax">10</span></div>
      </div>
...
// Try to auto-claim share reward if opened from a cast embed.
ensureSdk().then(() => maybeClaimShareReward()).catch(() => {});
